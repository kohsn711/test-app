...
## 概要
Supabase Authを使ったメール＋パスワードによるログイン。アカウントは管理者がSupabaseダッシュボードで作成する運用。
...
### アカウント管理
- アカウント作成はSupabaseダッシュボード（Authentication → Users → Add user）で管理者が行う
- アプリ内にサインアップ画面は設けない（クローズドな運用）
